...
class IMUGUI:
    def __init__(self, root):
        ...
        self.root.configure(bg='#f8f9fa')  # 밝은 회색 배경
        ...
        # 스마트 팩토리 색상 테마 - 밝은 버전
        self.colors = {
            'bg_dark': '#f8f9fa',      # 메인 배경 - 밝은 회색
            'bg_medium': '#ffffff',     # 카드 배경 - 흰색
            'bg_light': '#f1f3f5',      # 카드 내부 - 연한 회색
            'accent': '#e94560',        # 강조색 유지
            'success': '#00c896',       # 성공 - 밝은 녹색
            'warning': '#ff9800',       # 경고 - 주황색
            'danger': '#dc3545',        # 위험 - 빨간색
            'info': '#17a2b8',          # 정보 - 청록색
            'text_primary': '#212529',  # 주 텍스트 - 진한 회색
            'text_secondary': '#6c757d', # 보조 텍스트 - 중간 회색
            'grid': '#dee2e6'           # 그리드 - 연한 회색
        }
        ...
        # 밝은 테마 스타일 설정
        plt.style.use('default')  # 기본 밝은 스타일 사용
    ...
    def setup_header(self):
        ...
        # 자동 측정 버튼
        self.auto_btn = tk.Button(btn_container,
                                 text="🚀 AUTO MEASUREMENT",
                                 command=self.start_auto_collection,
                                 bg=self.colors['success'],
                                 fg='white',
                                 activebackground='#00a876',
                                 **button_config)
        ...
        tk.Button(btn_container,
                 ...
                 **button_config).pack(side='left', padx=3, pady=10)
    ...
    def create_status_indicator(self, parent, label, status):
        ...
        # 상태 LED (크기 증가)
        self.led_canvas = tk.Canvas(frame, width=24, height=24, 
                                   bg=self.colors['bg_light'], highlightthickness=0)
        self.led_canvas.pack(side='left', padx=5)
        
        led = self.led_canvas.create_oval(2, 2, 22, 22, 
                                         fill=self.colors['danger'], 
                                         outline=self.colors['danger'])
        
        # 상태 텍스트 - 크기 증가 및 볼드체 적용
    ...
    def setup_plots(self, parent):
        ...
        for i, ax in enumerate(self.axes):
            ax.set_facecolor('#fafafa')  # 매우 연한 회색 배경
            ...
            for spine in ax.spines.values():
                spine.set_edgecolor('#495057')  # 진한 회색 테두리
                spine.set_linewidth(1.5)  # 적당한 두께의 테두리
                spine.set_capstyle('round')  # 둥근 모서리
            
            # 그림자 효과를 위한 추가 테두리 (선택사항)
            ax.patch.set_edgecolor('#343a40')
    ...
    def clear_data(self):
        ...
        for ax in self.axes:
            ax.cla()
            ax.set_facecolor('#fafafa')  # 연한 회색 배경

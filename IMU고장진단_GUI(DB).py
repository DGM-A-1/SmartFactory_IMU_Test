...
class IMUGUI:
    ...
    def wait_for_connection(self, callback):
        ...
        def check():
            ...
            if self.ws_connected: callback(True)
            elif elapsed >= self.connection_timeout: callback(False)
            else: self.root.after(100, check)
        check()
    ...
    def stop_stream(self):
        ...
        if self.auto_mode:
            self.countdown_label.config(text=""); self.auto_mode = False
        else:
        ...
        try:
            ...
            for sensor_id, pred in sorted(self.predictions_data.items()):
                try:
                    ...
                    roll_val = pred.get("roll_drift")
                    pitch_val = pred.get("pitch_drift")
                    yaw_val = pred.get("yaw_drift")
                    serial = f"SENSOR-{sensor_id:02d}"
        ...
        try:
            ...
            for sn in range(8):
                ...
                if len(pred_vals) == 3:
                    ...
                    val = pred_vals[0]; is_faulty = abs(val) > self.threshold; status = "고장" if is_faulty else "정상"
                    self.predictions_data[sn] = {
                        'roll_drift': None, 'pitch_drift': None, 'yaw_drift': None,
                        'max_drift_axis': 'Unknown',
                        'max_drift_value': float(abs(val)), 'max_drift_signed': float(val),
                        'is_faulty': is_faulty, 'status': status
                    }
